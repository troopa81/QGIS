...
void QgsCreateMarkerItemMapTool::cadCanvasReleaseEvent( QgsMapMouseEvent *event )
{
  ...
  if ( !markerSymbol )
    markerSymbol.reset( qgis::down_cast<QgsMarkerSymbol *>( QgsSymbol::defaultSymbol( Qgis::GeometryType::Point ).release() ) );
...
void QgsCreateLineItemMapTool::lineCaptured( const QgsCurve *line )
{
  ...
  if ( qgsgeometry_cast<QgsCurve *>( geometry.get() ) )
  {
    ...
    if ( !lineSymbol )
      lineSymbol.reset( qgis::down_cast<QgsLineSymbol *>( QgsSymbol::defaultSymbol( Qgis::GeometryType::Line ).release() ) );
...
void QgsCreatePolygonItemMapTool::polygonCaptured( const QgsCurvePolygon *polygon )
{
  ...
  if ( qgsgeometry_cast<QgsCurve *>( geometry.get() ) )
  {
    ...
    if ( !fillSymbol )
      fillSymbol.reset( qgis::down_cast<QgsFillSymbol *>( QgsSymbol::defaultSymbol( Qgis::GeometryType::Polygon ).release() ) );
