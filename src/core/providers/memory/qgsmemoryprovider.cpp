  ...
  : QgsVectorDataProvider( uri, options, flags )
{
  ...
  setNativeTypes( QList< NativeType >()
                  << QgsVectorDataProvider::NativeType( tr( "Whole Number (integer)" ), QStringLiteral( "integer" ), QMetaType::Int, 0, 10 )
                  ...
                  // We know that double (QMetaType::Double) has only 15-16 significant numbers,
                  ...
                  << QgsVectorDataProvider::NativeType( tr( "Decimal Number (real)" ), QStringLiteral( "double" ), QMetaType::Double, 0, 32, 0, 30 )
                  << QgsVectorDataProvider::NativeType( tr( "Text (string)" ), QStringLiteral( "string" ), QMetaType::QString, 0, 255 )

                  // date type
                  << QgsVectorDataProvider::NativeType( QgsVariantUtils::typeToDisplayString( QMetaType::QDate ), QStringLiteral( "date" ), QMetaType::QDate, -1, -1, -1, -1 )
                  << QgsVectorDataProvider::NativeType( QgsVariantUtils::typeToDisplayString( QMetaType::QTime ), QStringLiteral( "time" ), QMetaType::QTime, -1, -1, -1, -1 )
                  << QgsVectorDataProvider::NativeType( QgsVariantUtils::typeToDisplayString( QMetaType::QDateTime ), QStringLiteral( "datetime" ), QMetaType::QDateTime, -1, -1, -1, -1 )

                  // integer types
                  << QgsVectorDataProvider::NativeType( tr( "Whole Number (smallint - 16bit)" ), QStringLiteral( "int2" ), QMetaType::Int, -1, -1, 0, 0 )
                  << QgsVectorDataProvider::NativeType( tr( "Whole Number (integer - 32bit)" ), QStringLiteral( "int4" ), QMetaType::Int, -1, -1, 0, 0 )
                  << QgsVectorDataProvider::NativeType( tr( "Whole Number (integer - 64bit)" ), QStringLiteral( "int8" ), QMetaType::LongLong, -1, -1, 0, 0 )
                  << QgsVectorDataProvider::NativeType( tr( "Decimal Number (numeric)" ), QStringLiteral( "numeric" ), QMetaType::Double, 1, 20, 0, 20 )
                  << QgsVectorDataProvider::NativeType( tr( "Decimal Number (decimal)" ), QStringLiteral( "decimal" ), QMetaType::Double, 1, 20, 0, 20 )

                  // floating point
                  << QgsVectorDataProvider::NativeType( tr( "Decimal Number (real)" ), QStringLiteral( "real" ), QMetaType::Double, -1, -1, -1, -1 )
                  << QgsVectorDataProvider::NativeType( tr( "Decimal Number (double)" ), QStringLiteral( "double precision" ), QMetaType::Double, -1, -1, -1, -1 )

                  // string types
                  << QgsVectorDataProvider::NativeType( tr( "Text, unlimited length (text)" ), QStringLiteral( "text" ), QMetaType::QString, -1, -1, -1, -1 )

                  // boolean
                  << QgsVectorDataProvider::NativeType( QgsVariantUtils::typeToDisplayString( QMetaType::Bool ), QStringLiteral( "boolean" ), QMetaType::Bool )

                  // blob
                  << QgsVectorDataProvider::NativeType( QgsVariantUtils::typeToDisplayString( QMetaType::QByteArray ), QStringLiteral( "binary" ), QMetaType::QByteArray )

                  // list types
                  << QgsVectorDataProvider::NativeType( QgsVariantUtils::typeToDisplayString( QMetaType::QStringList ), QStringLiteral( "stringlist" ), QMetaType::QStringList, 0, 0, 0, 0, QMetaType::QString )
                  << QgsVectorDataProvider::NativeType( QgsVariantUtils::typeToDisplayString( QMetaType::QVariantList, QMetaType::Int ), QStringLiteral( "integerlist" ), QMetaType::QVariantList, 0, 0, 0, 0, QMetaType::Int )
                  << QgsVectorDataProvider::NativeType( QgsVariantUtils::typeToDisplayString( QMetaType::QVariantList, QMetaType::Double ), QStringLiteral( "doublelist" ), QMetaType::QVariantList, 0, 0, 0, 0, QMetaType::Double )
                  << QgsVectorDataProvider::NativeType( QgsVariantUtils::typeToDisplayString( QMetaType::QVariantList, QMetaType::LongLong ), QStringLiteral( "integer64list" ), QMetaType::QVariantList, 0, 0, 0, 0, QMetaType::LongLong )

                  // complex types
                  << QgsVectorDataProvider::NativeType( QgsVariantUtils::typeToDisplayString( QMetaType::QVariantMap ), QStringLiteral( "map" ), QMetaType::QVariantMap, -1, -1, -1, -1 )
                  << QgsVectorDataProvider::NativeType( tr( "Geometry" ), QStringLiteral( "geometry" ), QMetaType::User )
                );
  ...
  if ( query.hasQueryItem( QStringLiteral( "field" ) ) )
  {
    ...
    for ( int i = 0; i < fields.size(); i++ )
    {
      ...
      if ( regularExpressionMatch.hasMatch() )
      {
        ...
        {
          if ( subType == QMetaType::UnknownType )
            subType = type;

          if ( type != QMetaType::QVariantList && type != QMetaType::QStringList )
            type = type == QMetaType::QString ? QMetaType::QStringList : QMetaType::QVariantList;
...
QString QgsMemoryProvider::dataSourceUri( bool expandAuthConfig ) const
{
  ...
  for ( int i = 0; i < attrs.size(); i++ )
  {
    ...
    bool isList = false;
    if ( field.type() == QMetaType::QVariantList || field.type() == QMetaType::QStringList )
    {
...
bool QgsMemoryProvider::addFeatures( QgsFeatureList &flist, Flags flags )
{
  ...
  for ( QgsFeatureList::iterator it = flist.begin(); it != flist.end() && result ; ++it )
  {
    ...
    if ( it->attributes().count() < fieldCount )
    {
      ...
      {
        attributes.append( QVariant( QMetaType( mFields.at( i ).type() ) ) );
      }
...
bool QgsMemoryProvider::addAttributes( const QList<QgsField> &attributes )
{
  ...
  for ( QgsField field : attributes )
  {
    ...
    NativeType nativeTypeCandidate( QString(), QString(), QMetaType::UnknownType );
    const QList<QgsVectorDataProvider::NativeType> nativeTypesList( nativeTypes() );
    for ( const NativeType &nativeType : nativeTypesList )
    {
      ...
      if ( nativeType.mType == field.type()
           && nativeTypeCandidate.mType == QMetaType::UnknownType )
        nativeTypeCandidate = nativeType;
    }
    if ( !isNativeTypeName )
    {
      if ( nativeTypeCandidate.mType == QMetaType::UnknownType )
      {
